...
export default function StartCourseButton({ courseId }: { courseId: number }) {
  ...
  async function handleStartCourse(e: MouseEvent<HTMLButtonElement>) {
    ...
    try {
      console.log(typeof participate)
      ...
      console.error(err)
      setStatus('error')
      setError('Failed to create participation')
    }
  }
  ...
  return (
    <div className="mt-6">
      <button
        ...
        disabled={isLoading}
        className={`relative inline-flex item-center gap-2 px-6 py-3 font-semibold rounded transition duration-300 ease-in-out
      ...
      {isError && (
        <p className="mt-2 text-sm text-red-400 flex item-center gap-2">
